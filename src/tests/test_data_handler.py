...
def test_execute_sql_with_return_id(test_db_call):
    ...
    """
    result, new_id = test_db_call.execute_sql(sql_statement, return_id=True)
    assert result is True
    assert new_id > 0
...
def test_read_database(test_db_call):
    ...
    # First insert a user
    test_db_call.add_user_name("read_test_user")
    # Then read it back
...
def test_read_database_with_params(test_db_call):
    """Test reading database with parameterized query"""
    # First insert a user
    test_db_call.add_user_name("param_test_user")
    # Then read it back with params
    sql_statement = "SELECT id, name FROM user WHERE name = ?"
    params = ("param_test_user",)
    results = test_db_call.read_database(sql_statement, params)
    assert isinstance(results, list)
    assert len(results) > 0
...
def test_build_database(test_db_call):
    ...
    for table in config.DATABASE_SCHEMA["tables"]:
        ...
        results = test_db_call.read_database(sql)
...
def test_add_user_name(test_db_call):
    ...
    results = test_db_call.read_database(sql, (user_name,))
